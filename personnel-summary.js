...
function isAnalyticalQuestion(value) {
  ...
  const mentionsGroup = /(ตำรวจ|บุคลากร|เจ้าหน้าที่|กำลังพล|ทำเนียบบุคลากร|ผู้นำตำบล|ผู้ใหญ่บ้าน|กำนัน)/u.test(text);
  const requestsAnalysis = /(วิเคราะห์|สรุป|จำนวน|กี่คน|เท่าไร|เท่าไหร่|มากที่สุด|น้อยที่สุด|เปรียบเทียบ|สัดส่วน|เปอร์เซ็นต์|%|คิดเป็น|แบ่งเป็น|จัดกำลัง|กระจายกำลัง|นับ|รวม)/u.test(text);
  const rejectsDetails = /(?:ไม่ต้อง|ไม่เอา|ไม่ต้องการ).{0,32}(?:รายชื่อ|ชื่อ|เบอร์|โทร|อีเมล)/u.test(text);
  const requestsDetails = /(?:ขอ|แสดง|ดึง|ส่ง|แจ้ง|บอก|เอา).{0,32}(?:รายชื่อ|ชื่อ|เบอร์|โทร|อีเมล)/u.test(text);
