...
const SHEET_PLACES    = 'ฐานข้อมูลพื้นที่ลานสัก'; // ฐานข้อมูลสถานที่/จุดสังเกตในพื้นที่ อ.ลานสัก
...
/**
 * โหลดฐานข้อมูลสถานที่/จุดสังเกตในพื้นที่ อ.ลานสัก จากแท็บ "ฐานข้อมูลพื้นที่ลานสัก"
 * คอลัมน์ A-AB ตามลำดับ: รหัสข้อมูล, ประเภทข้อมูล, ประเภทย่อย, ชื่อทางการ, ชื่อเรียกอื่น,
 * หมู่ที่, ชื่อหมู่บ้าน, ตำบล, อำเภอ, จังหวัด, รหัสไปรษณีย์, ที่อยู่เพิ่มเติม, ละติจูด, ลองจิจูด,
 * เบอร์โทร, เวลาเปิดปิด, รายละเอียด, จุดสังเกต, ระดับความเสี่ยง, ช่วงเวลาเสี่ยง, หน่วยรับผิดชอบ,
 * คำค้นหา, สถานะใช้งาน, สถานะตรวจสอบ, แหล่งที่มา, วันที่อัปเดต, ผู้บันทึก, ระดับการเข้าถึง
 */
async function fetchPlaces() {
  ...
  try {
    const response = await sheets.spreadsheets.values.get({
      spreadsheetId: SPREADSHEET_ID,
      range: `${SHEET_PLACES}!A2:AB2000`, // ข้ามแถวหัวตาราง (แถว 1)
    });
    const rows = response.data.values || [];
    return rows
      .filter(row => row[0]) // ต้องมีรหัสข้อมูล
      .map(row => ({
        code: row[0] || '',
        category: row[1] || '',
        subtype: row[2] || '',
    ...
    console.error('Error loading places from sheet:', err.message);
    return [];
  }
}
